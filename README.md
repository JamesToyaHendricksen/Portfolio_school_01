...
### デモモード（バックエンド停止時でも動作）
- URL の末尾に `?mode=demo` を付けると、フロントエンドがデモモードで動作します。
...
3. 自動デプロイ完了後、`/api/csrf` と `/api/sessions` を疎通確認
4. GitHub Pages 側でログイン・判定・イベント一覧を1回実施

補足:
- 無料プランはスリープ復帰直後にタイムアウトが起きる場合があります。
- 面接前は、必ず一度ログインと判定を実行してウォームアップしてください。
