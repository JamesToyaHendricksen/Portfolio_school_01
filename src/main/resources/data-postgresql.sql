...
INSERT INTO monitoring_settings (id, user_id, email_monitoring, sns_monitoring, network_monitoring, notification_enabled, notification_email, created_at, updated_at)
VALUES
...
INSERT INTO security_events (id, user_id, event_type, risk_level, status, channel, detected_at, reason, recommendation, created_at, updated_at, deleted_at)
VALUES
...
INSERT INTO reports (id, user_id, report_type, target_period_start, target_period_end, summary, generated_at, created_at, updated_at, deleted_at)
VALUES
...
INSERT INTO attachments (id, security_event_id, file_name, file_path, file_type, file_size, created_at, updated_at, deleted_at)
VALUES
...
INSERT INTO email_notifications (id, user_id, security_event_id, notification_type, recipient_email, sent_at, send_status, created_at, updated_at)
VALUES
...
SELECT setval(pg_get_serial_sequence('event_categories', 'id'), GREATEST((SELECT MAX(id) FROM event_categories), 1));
SELECT setval(pg_get_serial_sequence('monitoring_settings', 'id'), GREATEST((SELECT MAX(id) FROM monitoring_settings), 1));
SELECT setval(pg_get_serial_sequence('security_events', 'id'), GREATEST((SELECT MAX(id) FROM security_events), 1));
...
SELECT setval(pg_get_serial_sequence('email_notifications', 'id'), GREATEST((SELECT MAX(id) FROM email_notifications), 1));
